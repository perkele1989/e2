...
void main()
{
	...
	vec4 worldRoot = mesh.modelMatrix * vec4(0.0, 0.0, 0.0, 1.0);
	...
	worldVertex.x += cos(time.x*1.3 + ss*20.0) * mix(0.065 * ss * 0.25, 0.0, heightCoeff);
	worldVertex.z += sin(time.x*0.7 + ss*40.0) * mix(0.073 * ss * 0.25, 0.0, heightCoeff);
