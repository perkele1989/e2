...
void e2::UIContext::drawSDFTextCarousel(e2::FontFace fontFace, float fontSize, e2::UIColor color, glm::vec2 position, std::string const& markdownUtf8, float heightScale, float time)
{
	position.x = glm::floor(position.x);
	position.y = glm::floor(position.y);
	...
	for (uint32_t i = 0; i < src.size(); i++)
	{
		...
		float s = glm::sin(time - float(i) / 4.0f);
		float offset = s * 0.5 + 0.5;
		offset = glm::elasticEaseOut(offset);
		glyphOffset.y -= offset * heightScale;
